#!/usr/bin/env python3
import os, sys, json, sqlite3, subprocess, re, selectors
...
def _looks_interactive(cmd: str) -> bool:
    ...
    # sudo + common interactive tools (add more as you bump into them)
    interactive_keys = [
        "sudo ", "adduser", "useradd", "passwd", "chpasswd",
        "apt ", "apt-get ", "dpkg ", "nano", "vi", "vim", "less", "more",
        "ssh ", "ftp ", "scp ", "sftp ",
    ]
    return any(k in c for k in interactive_keys)

def run_command(cmd):
    print(f"{C.COMMAND}$ {cmd}{C.RESET}")

    # If it needs a real terminal (password prompts / y/n prompts), attach to TTY.
    if _looks_interactive(cmd):
        proc = subprocess.Popen(cmd, shell=True, text=True,
                                stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr)
        code = proc.wait()
        return {"cmd": cmd, "exit": code, "stdout": "", "stderr": ""}

    # Otherwise: stream stdout/stderr live AND still capture a short log.
    proc = subprocess.Popen(cmd, shell=True, text=True,
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1)

    sel = selectors.DefaultSelector()
    if proc.stdout: sel.register(proc.stdout, selectors.EVENT_READ, ("stdout",))
    if proc.stderr: sel.register(proc.stderr, selectors.EVENT_READ, ("stderr",))

    out_buf, err_buf = [], []
    MAX_CHARS = 8000  # keep logs sane
    ...
    while True:
        ...
        for key, _ in sel.select(timeout=0.1):
            ...
            if stream_name == "stdout":
                ...
                # keep stderr red like you already do
                print(f"{C.ERROR}{line.rstrip()}{C.RESET}", file=sys.stderr)
                err_buf.append(line)

        # stop once process ended and no streams left
        if proc.poll() is not None and not sel.get_map():
            break
    ...
    if len(stdout) > MAX_CHARS: stdout = stdout[-MAX_CHARS:]
    if len(stderr) > MAX_CHARS: stderr = stderr[-MAX_CHARS:]

    return {"cmd": cmd, "exit": code, "stdout": stdout.rstrip(), "stderr": stderr.rstrip()}
